...
SymbolTable* constructSymbolTable(TreeNode* root, SymbolTable* table = nullptr) {
    if(!table) {
        table = new SymbolTable;
    }
    if(root->node_kind == ID_NODE || root->node_kind == READ_NODE || root->node_kind == ASSIGN_NODE) {
        table->Insert(root->id, root->line_num);
    }
    TreeNode* child = root->child[0];
    for(int i=0; child; child = root->child[++i]) {
        constructSymbolTable(child, table);
    }
    TreeNode* sibling = root->sibling;
    if(sibling){
        constructSymbolTable(sibling, table);
    }
    return table;
}
